...
# DQN Agent
class ProjectAgent:
    def __init__(self, gamma=0.99, lr=1e-3, buffer_size=5000, batch_size=64, epsilon_start=1.0, epsilon_end=0.01, epsilon_decay=0.995):
...
# Training Loop
# episodes  = 20 % du buffer 
def train_dqn(env=env, episodes=1000, update_freq=5, prev_episods=0):
    ...
    for episode in range(episodes-prev_episods):
        ...
        while not done and not truncated:
            ...
            normalized_reward = np.clip(reward, reward_min, reward_max)
            agent.memory.add(state, action, normalized_reward, next_state, done)
...
if __name__ == "__main__":
    rewards, trained_agent = train_dqn(env=env, episodes=1000, update_freq=10, prev_episods=0) #100)
